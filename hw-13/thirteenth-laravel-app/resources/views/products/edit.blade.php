...
@section('content')
    ...
    <div class="container px-4 py-6">
        <h1 class="page-title text-2xl font-bold mb-4" style="margin-top:50px;">
            Редактировать товар #{{ $product->id }}
        </h1>

        <div id="msg" class="notice"></div>

        <form id="editForm" class="max-w-xl">
            <div class="form-field">
                <label for="sku">SKU</label>
                <input id="sku" name="sku" type="text" value="{{ $product->sku }}" required>
            ...
            <div class="form-field">
                ...
                <input id="price" name="price" type="number" step="0.001" min="0"
                       value="{{ number_format((float)$product->price, 3, '.', '') }}" required>
                <div class="error" data-err="price"></div>
            </div>

            <div class="flex gap-2">
                <button type="submit" class="btn btn-primary">Обновить</button>
                <a href="{{ route('products.show.page', $product) }}" class="btn btn-secondary">Отмена</a>
            </div>
        </form>
    </div>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    msg.textContent = data.message || 'Ошибка обновления';
                    return;
                }

                // Возвращается обновлённый продукт
                window.location.href = '{{ route('products.show.page', $product) }}';
            } catch (err) {
